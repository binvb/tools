...
async function render(
  start = 0,
  itemNum: number = props.initDataNum * 2,
  ...
  for (let i = 0, len = itemNum; i < len; i++) {
    ...
    if (_index >= 0 && _index < _maxLen) {
      ...
      await sleep(0);
      _offsetHeight = itemTemplate.value
        ? getOffsetHeight(itemTemplate.value as HTMLElement)
        : 0;

      getShowData(
        data.sourceData as ItemProps[],
        data.currentData,
        direction,
        _index,
        _offsetHeight,
...
function scrollHandler() {
  ...
  let _scrollItemNum = getScrollItemNum(
    data.currentData,
    _distance,
    _direction
  );
  ...
  if (_scrollItemNum > 0) {
    ...
    addQueue(() => {
      return render(
        _direction === "down"
          ? data.currentData[data.currentData.length - 1].index + 1
          : data.currentData[0].index - 1,
        _scrollItemNum,
        _direction
      );
    });
...
<template>
  ...
  >
    <li ref="itemTemplate">
      <component
        :is="props.ScrollItemComponent"
        :itemData="data.templateData"
      />
    </li>
  </ul>
</template>
